#!/bin/bash
#
# Copyright (C) 2025 The LineageOS Project
# SPDX-License-Identifier: Apache-2.0
#
# Setup makefiles script for Nothing Camera (NTCAM)
#

set -e

NTCAM_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROPRIETARY_FILES="${NTCAM_DIR}/proprietary-files.txt"
VENDOR_MK="${NTCAM_DIR}/ntcam-vendor.mk"
ANDROID_BP="${NTCAM_DIR}/Android.bp"

echo "[NTCAM] Generating makefiles from ${PROPRIETARY_FILES}..."

# 1. Generate ntcam-vendor.mk
cat << 'EOF' > "${VENDOR_MK}"
#
# Copyright (C) 2025 The LineageOS Project
# SPDX-License-Identifier: Apache-2.0
#
# Auto-generated file by setup-makefiles.sh. Do not edit directly.
#

NTCAM_PATH ?= hardware/nothing/camera

PRODUCT_SOONG_NAMESPACES += $(NTCAM_PATH)

PRODUCT_PACKAGES += \
    libcamera_client_shim \
    libui_shim \
    libarcsoft_hdr_detection \
    libarcsoft_mf_superresolution \
    libcpion \
    libtrustedapploader \
    libnelib \
    libnelib_vendor \
    libofflineproc_jni \
    vendor.noth.hardware.camera-V1-ndk \
    vendor.noth.hardware.camera-service-impl \
    vendor.noth.hardware.camera-service \
    vendor.noth.hardware.camera-service.xml \
    android.hardware.offline_proc@1.0-service.nothing \
    NothingExperience \
    NTCamera

PRODUCT_COPY_FILES += \
    $(NTCAM_PATH)/proprietary/system_ext/etc/permissions/privapp-permissions-NothingExperience.xml:$(TARGET_COPY_OUT_SYSTEM_EXT)/etc/permissions/privapp-permissions-NothingExperience.xml \
    $(NTCAM_PATH)/proprietary/vendor/etc/camera/NdotFont/Ndot-55.otf:$(TARGET_COPY_OUT_VENDOR)/etc/camera/NdotFont/Ndot-55.otf \
    $(NTCAM_PATH)/proprietary/vendor/etc/camera/Robotofont/Roboto-55.ttf:$(TARGET_COPY_OUT_VENDOR)/etc/camera/Robotofont/Roboto-55.ttf \
    $(NTCAM_PATH)/proprietary/vendor/etc/camera/SansCJKFont/NotoSansCJK.ttc:$(TARGET_COPY_OUT_VENDOR)/etc/camera/SansCJKFont/NotoSansCJK.ttc \
    $(NTCAM_PATH)/proprietary/vendor/etc/camera/ntcamoverridesettings.txt:$(TARGET_COPY_OUT_VENDOR)/etc/camera/ntcamoverridesettings.txt \
    $(NTCAM_PATH)/proprietary/vendor/etc/camera/ntcamperflocksettings.json:$(TARGET_COPY_OUT_VENDOR)/etc/camera/ntcamperflocksettings.json
EOF

# 2. Generate/Update Android.bp
cat << 'EOF' > "${ANDROID_BP}"
//
// Copyright (C) 2025 The LineageOS Project
// SPDX-License-Identifier: Apache-2.0
//
// Auto-generated Soong build declarations for NTCAM port
//

soong_namespace {
    imports: [
        "hardware/nothing",
    ],
}

// Camera Client Shim
cc_library_shared {
    name: "libcamera_client_shim",
    srcs: [
        "shim/camera_client_shim.cpp",
        "offlineproc/IOfflineProcService.cpp",
    ],
    shared_libs: [
        "libutils",
        "liblog",
        "libbinder",
    ],
    system_ext_specific: true,
}

// Subdirectories
subdirs = [
    "nothing-fwk",
    "offlineproc",
]

// Prebuilt Libraries & Binaries

cc_prebuilt_library_shared {
    name: "libarcsoft_hdr_detection",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/libarcsoft_hdr_detection.so",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_library_shared {
    name: "libarcsoft_mf_superresolution",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/libarcsoft_mf_superresolution.so",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_library_shared {
    name: "libcpion",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/libcpion.so",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_library_shared {
    name: "libtrustedapploader",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/libtrustedapploader.so",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_library_shared {
    name: "libofflineproc_jni",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/system_ext/lib64/libofflineproc_jni.so",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
                "libandroid",
                "libnativewindow",
                "libcamera_client_shim",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    system_ext_specific: true,
}

cc_prebuilt_library_shared {
    name: "libnelib",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/system_ext/lib64/libnelib.so",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    system_ext_specific: true,
}

cc_prebuilt_library_shared {
    name: "libnelib_vendor",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/libnelib.so",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_library_shared {
    name: "vendor.noth.hardware.camera-V1-ndk",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/vendor.noth.hardware.camera-V1-ndk.so",
            ],
            shared_libs: [
                "libbinder_ndk",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_library_shared {
    name: "vendor.noth.hardware.camera-service-impl",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/lib64/vendor.noth.hardware.camera-service-impl.so",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
                "libhardware",
                "libcamera_metadata",
                "vendor.noth.hardware.camera-V1-ndk",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
}

cc_prebuilt_binary {
    name: "vendor.noth.hardware.camera-service",
    owner: "nothing",
    strip: {
        none: true,
    },
    check_elf_files: false,
    target: {
        android_arm64: {
            srcs: [
                "proprietary/vendor/bin/hw/vendor.noth.hardware.camera-service",
            ],
            shared_libs: [
                "liblog",
                "libutils",
                "libcutils",
                "libbinder_ndk",
                "vendor.noth.hardware.camera-service-impl",
                "vendor.noth.hardware.camera-V1-ndk",
            ],
        },
    },
    compile_multilib: "64",
    prefer: true,
    soc_specific: true,
    init_rc: [
        "proprietary/vendor/etc/init/vendor.noth.hardware.camera-service.rc",
    ],
}

vintf_fragment {
    name: "vendor.noth.hardware.camera-service.xml",
    src: "proprietary/vendor/etc/vintf/manifest/vendor.noth.hardware.camera-service.xml",
    vendor: true,
}

// Applications

android_app_import {
    name: "NothingProxy",
    owner: "nothing",
    apk: "proprietary/system_ext/app/NothingProxy/NothingProxy.apk",
    certificate: "platform",
    dex_preopt: {
        enabled: false,
    },
    system_ext_specific: true,
}

android_app_import {
    name: "NothingExperience",
    owner: "nothing",
    apk: "proprietary/system_ext/priv-app/NothingExperience/NothingExperience.apk",
    certificate: "platform",
    dex_preopt: {
        enabled: false,
    },
    privileged: true,
    system_ext_specific: true,
}

android_app_import {
    name: "NTCamera",
    owner: "nothing",
    apk: "proprietary/system_ext/priv-app/NTCamera/NTCamera.apk",
    certificate: "platform",
    dex_preopt: {
        enabled: false,
    },
    privileged: true,
    system_ext_specific: true,
}
EOF

chmod +x "${VENDOR_MK}" 2>/dev/null || true
echo "[NTCAM] Setup makefiles complete!"
